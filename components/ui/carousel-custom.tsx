      ...
      >
        ...
        >
          ...
          <div className="flex justify-center">
            <Link href={href} className="mt-6 px-4 py-2 w-fit mx-auto sm:text-sm text-black bg-white h-12 border border-transparent text-xs flex justify-center items-center rounded-2xl hover:shadow-lg transition duration-200 shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)]">
              {button}
            </Link>
          </div>
        </article>
...
const CarouselControl = ({ type, title, handleClick }: CarouselControlProps) => {
  return (
    <Button
      variant="outline"
      size="icon"
      className={`w-10 h-10 rounded-full bg-background/80 backdrop-blur-sm hover:bg-background/90 border-primary/10 hover:border-primary/30 transition-all duration-200 ${
        type === "previous" ? "mr-2" : "ml-2"
      }`}
      title={title}
      onClick={handleClick}
    >
      {type === "previous" ? <ChevronLeft className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
    </Button>
  )
}
