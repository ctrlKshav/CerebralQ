      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <Card className="p-8">
          <div className="space-y-6">
            ...
            {/* Visual indicator for selected trait */}
            <div className="relative w-full h-12 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
              {/* Center line with z-index to ensure visibility */}
              <div className="absolute top-0 bottom-0 left-1/2 -translate-x-[1px] w-[2px] bg-gray-300 dark:bg-gray-600 z-10" />
              
              {/* 50% neutral label inside the larger indicator */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-xs text-gray-400 dark:text-gray-500 z-10">
                50% neutral
              </div>
              
              {/* Calculate deviation from 50% for the selected trait */}
              {(() => {
                const score = traitScores[selectedTrait];
                const deviation = Math.abs(score[`${score.dominant}Percentage`] - 50);
                const markerPosition = score.dominant === "left" 
                  ? 50 - (score.leftPercentage - 50) 
                  : 50 + (score.rightPercentage - 50);
                
                // Determine text alignment for the larger indicator
                const textAlignment = markerPosition <= 10 ? "left" 
                                   : markerPosition >= 90 ? "right" 
                                   : "center";
                ...
                return (
                  <>
                    {/* Position the percentage label above the large progress bar with adjusted alignment */}
                    <div 
                      className={`absolute -top-8 whitespace-nowrap z-20 text-${textAlignment}`}
                      style={{ 
                        left: `${markerPosition}%`,
                        transform: `translateX(${translateX})`,
                      }}
                    >
                      <span className="text-base font-medium" style={{ color: traitDescriptions[selectedTrait].color }}>
                    ...
                    {score.dominant === "left" && deviation > 0 && (
                      <div 
                        className="absolute top-0 h-full z-0 bg-gradient-to-l rounded-l-full flex items-center justify-center"
                        style={{
                          right: "50%",
                          width: `${deviation}%`,
                          backgroundColor: traitDescriptions[selectedTrait].color,
                        }}
                      >
                        {deviation > 15 && (
                          <span className="text-white font-bold text-xs md:text-sm pl-2 pr-2 z-20 relative">
                            {traitDescriptions[selectedTrait].leftLabel}
                          </span>
                        )}
                      </div>
                    ...
                    {score.dominant === "right" && deviation > 0 && (
                      <div 
                        className="absolute top-0 h-full z-0 bg-gradient-to-r rounded-r-full flex items-center justify-center"
                        style={{
                          left: "50%",
                          width: `${deviation}%`,
                          backgroundColor: traitDescriptions[selectedTrait].color,
                    ...
                    {/* Marker dot on the large progress bar */}
                    <div
                      className="absolute top-1/2 -translate-y-1/2 w-5 h-5 rounded-full border-2 bg-background z-20"
                      style={{
                        left: `${markerPosition}%`,
                        borderColor: traitDescriptions[selectedTrait].color,
