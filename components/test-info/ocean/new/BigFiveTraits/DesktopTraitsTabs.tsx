...
const DesktopTraitsTabs = ({
  traits,
  initialTrait,
}: DesktopTraitsTabsProps) => {
        ...
        <div className="sticky top-24">
          ...
          >
            {traits.map((trait) => {
              ...
              console.log(trait.gradient);
              return (
                <TabsTrigger
                  key={trait.name}
                  value={trait.name}
                  className={`flex h-auto w-full items-center justify-start p-4 rounded-lg transition-all duration-200 text-base data-[state=active]:text-white data-[state=active]:bg-none}
                    ${
                      isActive
                        ? `shadow-sm ${trait.gradient} `
                        : "hover:bg-muted/80 text-foreground"
                    }`}
                  aria-selected={isActive}
                >
                  <div
                    className={`mr-4 flex items-center justify-center w-10 h-10 rounded-md text-white
                    ${isActive ? `  bg-white/20` : `${trait.gradient}`}
                    `}
                  >
                    <TraitSvgIcon
